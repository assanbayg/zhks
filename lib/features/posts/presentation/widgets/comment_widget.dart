...
class CommentWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final formattedDate = DateFormat('dd.MM.yy').format(DateTime.parse(comment.createdAt));
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text(
                    formattedDate,
                    style: context.texts.bodySmall.copyWith(color: primaryColors.gray),
                  ),
                  ElevatedButton.icon(
                    onPressed: () async {
                      if (comment.isLikedByUser) {
                        await ref.read(unlikeCommentProvider(comment.id).future);
                      } else {
                        await ref.read(likeCommentProvider(comment.id).future);
                      }
                    },
                    icon: Icon(
                      comment.isLikedByUser ? Icons.favorite : Icons.favorite_outline,
                    ),
                    label: Text(comment.likesCount.toString()),
                    style: ElevatedButton.styleFrom(
                      backgroundColor:
                          comment.isLikedByUser ? tertiaryColors.red : tertiaryColors.gray,
                      foregroundColor:
                          comment.isLikedByUser ? primaryColors.red : primaryColors.black,
